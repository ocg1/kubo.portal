...
/*HotJar*/
		  ...
		  (function(h,o,t,j,a,r){
		        ...
		        h._hjSettings={hjid:39318,hjsv:5};
...
define(function () {
	...
	/*Pixel para medir el deseñpeño del sitio
	...
	;NREUM.info={beacon:"bam.nr-data.net",errorBeacon:"bam.nr-data.net",licenseKey:"89865c3038",applicationID:"8608251",sa:1,agent:"js-agent.newrelic.com/nr-632.min.js"}
...
$(document).ready(function(){
	tagMananger ();
	$('head').append("<noscript><iframe src='https://www.googletagmanager.com/ns.html?id=GTM-WWFJWG5' height='0' width='0' style='display:none;visibility:hidden'></iframe></noscript>");
});
function GTM_eventos (eventoGTM){
	try {
		
	   dataLayer.push({'event': eventoGTM});
	}
	catch(err) {
...
function facebook_events (event ){
	try {
		if(event == "DatosPersonales" || event == "Publicacion" ){
			window._fbq.push(["track", event, {}]);
		}else {
			window._fbq.push(["trackCustom", event, {}]);
		}
...
function googleEvents (categoria, action, label){
	try {
		ga('send', 'event', categoria, action, label);
	}
	catch(err) {
...
function gaVirtualPages (page){
	...
	try {
		...
		mixpanel.init("b737252ef9e5b997516ea7df624fb1c6");
		/*----end Mixpanel --*/
	}
	catch(err) {	
