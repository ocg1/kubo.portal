...
import mx.com.kubo.notificaciones.parsers.NotificacionPeticionCambioPassword;
import mx.com.kubo.notificaciones.receptores.ReceptorIMP;

public class NotificablePeticionCambioPassword extends NotificableEMO
implements NotificableIMO 
{
	public NotificablePeticionCambioPassword(Membership emisor)
	{
		this.emisor     = emisor;
		this.acreditado = emisor;
		
		receptores    = new ReceptorIMP(Evento.PETICION_CAMBIO_PASSWORD, emisor);
		conexion_SMTP = new ConexionIMP(receptores.getSMTPServer());
		
		notificacion  = new NotificacionPeticionCambioPassword(conexion_SMTP, emisor);
	...
	public final void notificar() throws NotificacionException
	{
		...
		notificar_receptores_por_iteracion(Evento.PETICION_CAMBIO_PASSWORD);	
	}
}
