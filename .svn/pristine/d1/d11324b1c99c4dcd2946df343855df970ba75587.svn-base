...
import mx.com.kubo.notificaciones.parsers.NotificacionPeticionCambioCuentaCorreo;
import mx.com.kubo.notificaciones.receptores.ReceptorIMP;

public class NotificablePeticionCambioCuentaCorreo extends NotificableEMO
implements NotificableIMO 
{
	public NotificablePeticionCambioCuentaCorreo(Membership emisor)
	{
		this.emisor     = emisor;
		this.acreditado = emisor;
		
		receptores    = new ReceptorIMP(Evento.PETICION_CAMBIO_CUENTA_CORREO, emisor);
		conexion_SMTP = new ConexionIMP(receptores.getSMTPServer());
		
		notificacion  = new NotificacionPeticionCambioCuentaCorreo(conexion_SMTP, emisor);
	...
	{		
		...
		notificar_receptores_por_iteracion(Evento.PETICION_CAMBIO_CUENTA_CORREO);
	}
}
