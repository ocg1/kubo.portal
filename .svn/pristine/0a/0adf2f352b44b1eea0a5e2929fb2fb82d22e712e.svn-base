  ...
  $.fn.smartTab = function(action) {
    ...
    return this.each(function() {
      ...
      function setTabs(){
        ...
        $(tabs).each(function(){
          var elm = $($(this).attr("data-tab"), obj);
        ...
        if(options.contentCache && selTab.data('hasContent')){
          ...
          $.ajax({
              ...
              success: function(res){
                if(res && res.length>0){ 
                  $(selTab.attr("data-tab"), obj).html(res);
      ...
      function animateTab(idx){
        ...
        if(options.transitionEffect == 'hslide'){ // horizontal slide
            ...
            if(curTab.length>0){
              $(curTab.attr("data-tab"), obj).css("left",cFLeft).animate({left:cLLeft},options.transitionSpeed,options.transitionEasing,function(e){
                  $(this).hide();
              });                     
            }                     
            $(selTab.attr("data-tab"),obj).css("left",sFLeft).width(cWidth).show().animate({left:sLLeft},options.transitionSpeed,options.transitionEasing,function(e){
              $(this).show();
            ...
            var curElm = $(curTab.attr("data-tab"), obj);
            var selElm = $(selTab.attr("data-tab"), obj);
            if(idx>obj.data('curTabIdx')){ // forward
              cFTop = 0;
            ...
            if(curTab.length>0){
              $(curTab.attr("data-tab"), obj).slideUp(options.transitionSpeed,options.transitionEasing,function(){
                  $(selTab.attr("data-tab"), obj).slideDown(options.transitionSpeed,options.transitionEasing,function(){
                    setTabAnchor(idx,curTab,selTab);
                  });
              });
            }else{
            ...
            $($(curTab).attr("data-tab"), obj).fadeOut(options.transitionSpeed,options.transitionEasing,function(){
              $($(selTab).attr("data-tab"), obj).fadeIn(options.transitionSpeed,options.transitionEasing);
                $(this).show();
                setTabAnchor(idx,curTab,selTab);
            });
        }else{ // none
            if(curTab.length>0) $(curTab.attr("data-tab"),obj).hide();
            $(selTab.attr("data-tab"),obj).show();
            setTabAnchor(idx,curTab,selTab);
        }
      ...
      function adjustHeight(selTab){
        ...
        if(options.autoHeight){
          tabContainer.animate({height: $(selTab.attr("data-tab")).height()}, options.transitionSpeed);
        } 
      }
